...
class GeneralShape {
    ...
    // setColor function to check if input color value is valid.
    setColor(inputColor) {
        if (validColors.includes(inputColor) || hexPattern.test(inputColor)) {
            return inputColor
        }
    }
    ...
    invalidTemp() {
        ...
        <text x="150" y="125" font-size="15" text-anchor="middle" fill="red"> Invalid color/characters inputs! Please try again</text>
    </svg>`
    }
}
...
class Triangle extends GeneralShape {
    ...
    svgTemp() {
        if (this.setColor(this.shapeColor) && this.setColor(this.textColor) && this.testLetter(this.characters)) {
